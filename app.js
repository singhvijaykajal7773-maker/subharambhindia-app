...
async function bootstrap() {
  ...
  try {
    ...
    ME = res.user;
    if (res.token) { TOKEN = res.token; localStorage.setItem('sai_token', res.token); }
    applyRoleUI();
    $('#auth-screen').classList.add('hidden');
    $('#app-screen').classList.remove('hidden');
    document.body.classList.add('app-active');
    ...
    // Do not log the user out because the server is temporarily unavailable.
    // Only a real authentication failure (401/403) clears the saved session.
...
function connectSocket() {
  ...
  socket.on('message:poll-updated', ({messageId,attachmentData}) => { const el=document.querySelector(`[data-msg-id="${messageId}"]`); if(el){const card=el.querySelector('.poll-card');if(card)card.outerHTML=renderPollCard(messageId,attachmentData);} });
  socket.on('message:event-updated', ({messageId,attachmentData}) => { const el=document.querySelector(`[data-msg-id="${messageId}"]`); if(el){const card=el.querySelector('.event-card');if(card)card.outerHTML=renderEventCard(messageId,attachmentData);} });
  ...
  socket.on('call:offer', async (data) => {
    if(!ACTIVE_CALL || ACTIVE_CALL.callId!==data.callId){ PENDING_OFFERS.set(data.callId,data); return; }
...
function stopCallTimer(){ if(CALL_TIMER_INTERVAL){clearInterval(CALL_TIMER_INTERVAL);CALL_TIMER_INTERVAL=null;} CALL_STARTED_AT=null; }
function startCallTimer(){ stopCallTimer(); CALL_STARTED_AT=Date.now(); const tick=()=>{if(!CALL_STARTED_AT)return; const sec=Math.floor((Date.now()-CALL_STARTED_AT)/1000); const m=String(Math.floor(sec/60)).padStart(2,'0'), s=String(sec%60).padStart(2,'0'); $('#call-timer').textContent=`${m}:${s}`;}; tick(); CALL_TIMER_INTERVAL=setInterval(tick,1000); }
function resetCallUI(){
  ...
  $('#call-mute').textContent='🎤 Mute'; $('#call-camera').textContent='📷 Camera'; $('#call-timer').textContent='00:00';
  $('#call-incoming').classList.add('hidden'); $('#call-active').classList.remove('hidden');
}
...
async function acceptOffer(data){
  ...
  await flushPendingIce(data.callId);
  const answer=await PEER.createAnswer(); await PEER.setLocalDescription(answer);
  socket.emit('call:answer',{callId:data.callId,chatId:data.chatId,targetUserId:data.fromUserId,answer:PEER.localDescription});
  $('#call-status').textContent='Connected'; startCallTimer();
}
async function preparePeer(targetUserId,chatId,callId,kind){
  let cfg={iceServers:[{urls:'stun:stun.l.google.com:19302'}]};
  try{ const d=await api('/calls/config'); if(Array.isArray(d.iceServers)&&d.iceServers.length) cfg={iceServers:d.iceServers}; }catch(e){console.warn('Using default WebRTC ICE configuration',e);}
  ...
  PEER.onconnectionstatechange=()=>{ const state=PEER?.connectionState; if(state==='connected'){ $('#call-status').textContent='Connected'; if(!CALL_STARTED_AT) startCallTimer(); } if(state==='failed'){ $('#call-status').textContent='Connection failed'; } if(state==='disconnected') $('#call-status').textContent='Connection interrupted'; };
...
async function startCall(kind){
  ...
  if(ACTIVE_CALL) return alert('A call is already active.');
  ...
  $('#call-title').textContent=`${kind==='video'?'Video':'Voice'} call — ${targetName}`; $('#call-status').textContent='Calling…'; $('#call-overlay').classList.remove('hidden'); $('#call-incoming').classList.add('hidden'); $('#call-active').classList.remove('hidden');
  try{ await preparePeer(targetUserId,ACTIVE_CHAT_ID,callId,kind); socket.emit('call:start',{callId,chatId:ACTIVE_CHAT_ID,targetUserId,kind}); const offer=await PEER.createOffer(); await PEER.setLocalDescription(offer); socket.emit('call:offer',{callId,chatId:ACTIVE_CHAT_ID,targetUserId,offer:PEER.localDescription}); }
  catch(e){ alert(e.message); endCall(true,'Could not start call'); }
}
async function handleIncomingCall(data){
  ...
  $('#call-title').textContent=`${data.kind==='video'?'Incoming Video':'Incoming Audio'} Call`;
  $('#incoming-caller-name').textContent=data.callerName||'User'; $('#incoming-caller-avatar').src=data.callerAvatar||'/logo.png';
  $('#call-status').textContent='Incoming call'; $('#call-overlay').classList.remove('hidden'); $('#call-active').classList.add('hidden'); $('#call-incoming').classList.remove('hidden');
  const offer=PENDING_OFFERS.get(data.callId); if(offer) PENDING_OFFERS.delete(data.callId);
  $('#incoming-accept').onclick=async()=>{
    $('#incoming-accept').disabled=true; $('#incoming-reject').disabled=true;
    try{ $('#call-incoming').classList.add('hidden'); $('#call-active').classList.remove('hidden'); $('#call-status').textContent='Connecting…'; await preparePeer(data.callerId,data.chatId,data.callId,data.kind); if(offer) await acceptOffer(offer); }
    catch(e){ alert(e.message); endCall(true,'Could not accept call'); }
    finally{ $('#incoming-accept').disabled=false; $('#incoming-reject').disabled=false; }
  };
  $('#incoming-reject').onclick=()=>endCall(true,'Declined');
}
async function switchCamera(){
  const track=LOCAL_STREAM?.getVideoTracks?.()[0]; if(!track||!PEER)return;
  const settings=track.getSettings(); const next=settings.facingMode==='environment'?'user':'environment';
  try{ const stream=await navigator.mediaDevices.getUserMedia({audio:false,video:{facingMode:{ideal:next}}}); const nextTrack=stream.getVideoTracks()[0]; const sender=PEER.getSenders().find(x=>x.track?.kind==='video'); if(sender) await sender.replaceTrack(nextTrack); track.stop(); LOCAL_STREAM.removeTrack(track); LOCAL_STREAM.addTrack(nextTrack); $('#local-video').srcObject=LOCAL_STREAM; }
  catch(e){ alert('Could not switch camera. Your device may not support multiple cameras.'); }
}
function endCall(emit=true,reason='Call ended'){
  ...
  if(ACTIVE_CALL){ PENDING_ICE.delete(ACTIVE_CALL.callId); PENDING_OFFERS.delete(ACTIVE_CALL.callId); }
  LOCAL_STREAM?.getTracks().forEach(t=>t.stop()); LOCAL_STREAM=null; PEER?.close(); PEER=null; ACTIVE_CALL=null; resetCallUI();
}
...
$('#call-camera')?.addEventListener('click',()=>{const t=LOCAL_STREAM?.getVideoTracks()[0]; if(!t)return; t.enabled=!t.enabled; $('#call-camera').textContent=t.enabled?'📷 Camera Off':'🚫 Camera On';});
$('#call-switch-camera')?.addEventListener('click',switchCamera);
...
function renderPollCard(messageId,data){
  const options=Array.isArray(data?.options)?data.options:[];const votes=data?.votes||{};const counts=options.map((_,i)=>Object.values(votes).filter(v=>String(v)===String(i)).length);
  return `<div class="msg-attachment-card poll-card"><b>📊 ${escapeHtml(data?.question||'Poll')}</b>${options.map((o,i)=>`<div class="poll-option" data-poll-id="${escapeHtml(messageId)}" data-option="${i}"><span>${escapeHtml(o)}</span><strong>${counts[i]}</strong></div>`).join('')}</div>`;
}
function renderEventCard(messageId,data){
  const r=data?.rsvps||{};return `<div class="msg-attachment-card event-card"><b>📅 ${escapeHtml(data?.title||'Event')}</b><div>${escapeHtml(data?.date||'')} ${escapeHtml(data?.time||'')}</div><div>${escapeHtml(data?.location||'')}</div>${data?.link?`<a href="${escapeHtml(data.link)}" target="_blank" rel="noopener">Open link</a>`:''}<div class="event-rsvp"><button data-event-id="${escapeHtml(messageId)}" data-rsvp="going">Going (${Object.values(r).filter(x=>x==='going').length})</button><button data-event-id="${escapeHtml(messageId)}" data-rsvp="maybe">Maybe (${Object.values(r).filter(x=>x==='maybe').length})</button><button data-event-id="${escapeHtml(messageId)}" data-rsvp="not_going">Can't go (${Object.values(r).filter(x=>x==='not_going').length})</button></div></div>`;
}
function renderAttachment(msg){
  let html='';
  if(msg.mediaUrl){
    if(msg.mediaType==='image') html=`<div class="msg-attachment-card"><img src="${escapeHtml(msg.mediaUrl)}" alt="" loading="lazy"></div>`;
    else if(msg.mediaType==='video') html=`<div class="msg-attachment-card"><video src="${escapeHtml(msg.mediaUrl)}" controls playsinline></video></div>`;
    else if(msg.mediaType==='audio') html=`<div class="msg-attachment-card"><audio src="${escapeHtml(msg.mediaUrl)}" controls></audio></div>`;
    else html=`<div class="msg-attachment-card"><a class="doc-link" href="${escapeHtml(msg.mediaUrl)}" target="_blank" rel="noopener">📄 ${escapeHtml(msg.attachmentData?.originalName||'Open document')}</a></div>`;
  }
  if(msg.attachmentType==='contact'&&msg.attachmentData) html+=`<div class="msg-attachment-card"><b>👤 ${escapeHtml(msg.attachmentData.name||'Contact')}</b><div>${escapeHtml(msg.attachmentData.phone||'')}</div></div>`;
  if(msg.attachmentType==='poll') html+=renderPollCard(msg.id,msg.attachmentData);
  if(msg.attachmentType==='event') html+=renderEventCard(msg.id,msg.attachmentData);
  if(msg.attachmentType==='sticker'&&msg.mediaUrl) html=`<div class="msg-attachment-card"><img src="${escapeHtml(msg.mediaUrl)}" alt="Sticker" loading="lazy" style="max-width:160px;border:0"></div>`;
  return html;
}
function bindAttachmentInteractions(row,msg){
  row.querySelectorAll('.poll-option').forEach(x=>x.addEventListener('click',()=>socket.emit('message:poll-vote',{messageId:x.dataset.pollId,optionIndex:Number(x.dataset.option)})));
  row.querySelectorAll('[data-rsvp]').forEach(x=>x.addEventListener('click',()=>socket.emit('message:event-rsvp',{messageId:x.dataset.eventId,response:x.dataset.rsvp})));
}
function renderMessage(msg) {
  ...
  const sender = window.__currentChat?.type === 'group' && !mine ? `<span class="sender-name">${escapeHtml(getUserName(msg.senderId))}</span>` : '';
  row.innerHTML = `<div class="bubble">${sender}<span class="msg-text">${escapeHtml(msg.text||'')}</span>${renderAttachment(msg)}<span class="reactions">${formatReactions(msg.reactions)}</span><div class="meta">${fmtTime(msg.createdAt)}${mine ? (msg.readBy.length > 1 ? ' ✓✓' : ' ✓') : ''}</div></div>`;
  bindAttachmentInteractions(row,msg);
  row.addEventListener('dblclick', () => socket.emit('message:react', { messageId: msg.id, emoji: '❤️' }));
...
function sendMessage(){const input=$('#message-input');const text=input.value.trim();if(!text||!ACTIVE_CHAT_ID)return;socket.emit('message:send',{chatId:ACTIVE_CHAT_ID,text});input.value='';}
function closeAttachmentMenu(){$('#attachment-menu')?.classList.add('hidden');}
$('#btn-attach')?.addEventListener('click',e=>{e.stopPropagation();$('#attachment-menu')?.classList.toggle('hidden')});
document.addEventListener('click',e=>{if(!e.target.closest('.attach-wrap'))closeAttachmentMenu()});
async function uploadAndSend(file,attachmentType='file'){
  if(!file||!ACTIVE_CHAT_ID)return;const form=new FormData();form.append('file',file);try{const res=await api('/upload',{method:'POST',body:form});socket.emit('message:send',{chatId:ACTIVE_CHAT_ID,text:'',mediaUrl:res.url,mediaType:res.mediaType,attachmentType,attachmentData:{originalName:res.originalName,mimeType:res.mimeType,size:res.size}});}catch(err){alert('Upload failed: '+err.message)}
}
$('#file-input').addEventListener('change',e=>{const f=e.target.files[0];uploadAndSend(f,'document');e.target.value='';});
$('#media-input').addEventListener('change',e=>{const f=e.target.files[0];uploadAndSend(f,f?.type?.startsWith('video/')?'video':'image');e.target.value='';});
$('#camera-input').addEventListener('change',e=>{const f=e.target.files[0];uploadAndSend(f,f?.type?.startsWith('video/')?'video':'image');e.target.value='';});
$('#sticker-input').addEventListener('change',e=>{const f=e.target.files[0];uploadAndSend(f,'sticker');e.target.value='';});
$$('[data-attach]').forEach(btn=>btn.addEventListener('click',async()=>{
  const type=btn.dataset.attach;closeAttachmentMenu();
  if(type==='document'){const i=$('#file-input');i.accept='.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.zip';i.click();}
  else if(type==='media')$('#media-input').click();
  else if(type==='camera')$('#camera-input').click();
  else if(type==='sticker')$('#sticker-input').click();
  else if(type==='audio'){
    ...
    try{const stream=await navigator.mediaDevices.getUserMedia({audio:true});const rec=new MediaRecorder(stream);const chunks=[];rec.ondataavailable=e=>e.data.size&&chunks.push(e.data);rec.onstop=async()=>{stream.getTracks().forEach(t=>t.stop());const blob=new Blob(chunks,{type:rec.mimeType||'audio/webm'});await uploadAndSend(new File([blob],'voice-message.webm',{type:blob.type}),'audio')};rec.start();noticeAudioRecording(rec);}catch(e){alert('Microphone permission is required to record audio.');}
  }
  else if(type==='contact'){
    try{const r=await api('/users');openModal(`<h3>Share Contact</h3><div id="share-contact-list">${r.users.map(u=>`<button class="contact-share-row" data-id="${u.id}" data-name="${escapeHtml(u.name)}" data-phone="${escapeHtml(u.phone||'')}">👤 ${escapeHtml(u.name)}<small>${escapeHtml(u.phone||'')}</small></button>`).join('')}</div>`);$$('.contact-share-row').forEach(b=>b.addEventListener('click',()=>{socket.emit('message:send',{chatId:ACTIVE_CHAT_ID,text:'',attachmentType:'contact',attachmentData:{userId:b.dataset.id,name:b.dataset.name,phone:b.dataset.phone}});closeModal();}));}catch(e){alert(e.message)}
  }
  else if(type==='poll'){
    openModal(`<h3>Create Poll</h3><input id="poll-question" placeholder="Question"><input class="poll-input" placeholder="Option 1"><input class="poll-input" placeholder="Option 2"><input class="poll-input" placeholder="Option 3 (optional)"><button id="poll-send" class="btn-primary">Send Poll</button>`);$('#poll-send').onclick=()=>{const q=$('#poll-question').value.trim();const options=[...$$('.poll-input')].map(x=>x.value.trim()).filter(Boolean);if(!q||options.length<2)return alert('Question and at least 2 options are required.');socket.emit('message:send',{chatId:ACTIVE_CHAT_ID,text:'',attachmentType:'poll',attachmentData:{question:q,options,votes:{}}});closeModal();};
  }
  else if(type==='event'){
    openModal(`<h3>Share Event / Class</h3><input id="share-event-title" placeholder="Class / Seminar name"><input id="share-event-date" type="date"><input id="share-event-time" placeholder="Time"><input id="share-event-location" placeholder="Location"><input id="share-event-link" placeholder="Zoom / Meeting link"><button id="event-send" class="btn-primary">Share Event</button>`);$('#event-send').onclick=()=>{const title=$('#share-event-title').value.trim();if(!title)return alert('Event name is required.');socket.emit('message:send',{chatId:ACTIVE_CHAT_ID,text:'',attachmentType:'event',attachmentData:{title,date:$('#share-event-date').value,time:$('#share-event-time').value,location:$('#share-event-location').value,link:$('#share-event-link').value,rsvps:{}}});closeModal();};
  }
}));
function noticeAudioRecording(rec){const btn=$('#btn-attach');if(!btn)return;btn.textContent='⏹️';btn.title='Stop recording';const stop=()=>{if(rec.state!=='inactive')rec.stop();btn.textContent='📎';btn.title='Attachments';btn.removeEventListener('click',stop);};btn.addEventListener('click',stop,{once:true});}
