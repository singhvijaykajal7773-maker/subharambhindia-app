...
const OWNER_PHONE = String(process.env.OWNER_PHONE || '').replace(/\s+/g, '');
...
app.post('/api/auth/verify-otp', (req, res) => {
  ...
  if (existingUser) { if (OWNER_PHONE && existingUser.phone === OWNER_PHONE) existingUser.role='owner'; db.save(); return res.json({ status: 'login', token: signToken(existingUser.id), user: publicUser(existingUser) }); }
...
function isOwner(req){ return req.user?.role === 'owner' || (OWNER_PHONE && req.user?.phone === OWNER_PHONE); }
...
function userByPhone(phone){ return db.data.users.find(u=>String(u.phone||'').replace(/\s+/g,'')===String(phone||'').replace(/\s+/g,'')); }
