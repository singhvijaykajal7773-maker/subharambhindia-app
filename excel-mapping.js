...
function readWorkbook(filePath){
  ...
  if (ext === '.csv') {
    ...
    if (parsed.errors && parsed.errors.length) {
      ...
      throw new Error(`CSV parse error at row ${first.row ?? 'unknown'}: ${first.message || 'invalid CSV'}`);
    }
    return {sheetName: require('path').basename(filePath), rows: parsed.data.map(mapRow)};
  }
