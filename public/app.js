...
function applyRoleUI() { const owner = ME?.role === 'owner'; $$('.owner-only').forEach(el => el.classList.toggle('hidden', !owner)); }
...
function connectSocket() {
  ...
  socket.on('call:offer', async (data) => { if(!ACTIVE_CALL || ACTIVE_CALL.callId!==data.callId) return; await PEER?.setRemoteDescription(new RTCSessionDescription(data.offer)); const answer=await PEER.createAnswer(); await PEER.setLocalDescription(answer); socket.emit('call:answer',{callId:data.callId,chatId:data.chatId,targetUserId:data.fromUserId,answer:PEER.localDescription}); $('#call-status').textContent='Connected'; });
  socket.on('call:answer', async (data) => { if(!ACTIVE_CALL || ACTIVE_CALL.callId!==data.callId) return; await PEER?.setRemoteDescription(new RTCSessionDescription(data.answer)); $('#call-status').textContent='Connected'; });
  socket.on('call:ice', async (data) => { if(!ACTIVE_CALL || ACTIVE_CALL.callId!==data.callId) return; try{ await PEER?.addIceCandidate(new RTCIceCandidate(data.candidate)); }catch(e){ console.warn('ICE candidate failed',e); } });
