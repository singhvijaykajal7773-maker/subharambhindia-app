...
// DEMO_MODE is intentionally allowed in hosted/demo deployments.
// Demo OTP and demo login password are enabled only when DEMO_MODE=true.
...
const STUN_SERVERS = String(process.env.WEBRTC_STUN_URLS || 'stun:stun.l.google.com:19302').split(',').map(x=>x.trim()).filter(Boolean);
const TURN_SERVERS = String(process.env.WEBRTC_TURN_URLS || '').split(',').map(x=>x.trim()).filter(Boolean);
const TURN_USERNAME = String(process.env.WEBRTC_TURN_USERNAME || '');
const TURN_CREDENTIAL = String(process.env.WEBRTC_TURN_CREDENTIAL || '');
...
const WHATSAPP_API_VERSION = String(process.env.WHATSAPP_API_VERSION || '').trim();
...
const WHATSAPP_ENABLED = Boolean(WHATSAPP_ACCESS_TOKEN && WHATSAPP_PHONE_NUMBER_ID && WHATSAPP_API_VERSION);
...
function publicUser(u) { if (!u) return null; const { passwordHash, ...rest } = u; return { ...rest, online: typeof onlineUsers !== 'undefined' ? onlineUsers.has(u.id) : false }; }
function signToken(userId) { return jwt.sign({ userId }, JWT_SECRET, { expiresIn: '30d', issuer: 'subharambhindia', audience: 'subharambhindia-app' }); }
async function authMiddleware(req, res, next) {
  try { await db.ready; } catch { return res.status(503).json({ error: 'Database is not ready. Please try again.' }); }
...
const upload = multer({ storage, limits: { fileSize: 50 * 1024 * 1024, files: 1 }, fileFilter: (req, file, cb) => {
...
app.post('/api/auth/login-password', async (req, res) => {
  ...
  const validPassword = user && (await bcrypt.compare(suppliedPassword, user.passwordHash));
  const demoPasswordValid = DEMO_MODE && suppliedPassword === '123456';
  if (!user || (!validPassword && !demoPasswordValid)) return res.status(401).json({ error: 'Invalid credentials' });
  res.json({ token: signToken(user.id), user: publicUser(user) });
});
app.get('/api/auth/me', authMiddleware, (req,res)=>res.json({user:publicUser(req.user), token: signToken(req.user.id)}));
...
app.post('/api/upload',authMiddleware,(req,res,next)=>upload.single('file')(req,res,err=>{
  ...
  res.json({url:`/uploads/${req.file.filename}`,mediaType,originalName:req.file.originalname,size:req.file.size,mimeType:req.file.mimetype});
}));
...
app.post('/api/marketing/campaigns',authMiddleware,requireBusinessPermission('campaigns'),(req,res)=>{
  ...
  const mode=String(req.body.mode||'IN_APP').toUpperCase();
  if(!['IN_APP','WHATSAPP'].includes(mode))return res.status(400).json({error:'Invalid campaign mode'});
  const c={id:uuidv4(),ownerId,name,message:message.slice(0,10000),templateId,eventId,filters,contactIds,
    mediaUrl:String(req.body.mediaUrl||'').trim().slice(0,2048)||null,mediaType:String(req.body.mediaType||'').slice(0,50)||null,
    scheduledAt:req.body.scheduledAt?String(req.body.scheduledAt):null,mode,status:'draft',createdAt:Date.now(),
    whatsappTemplateName:String(req.body.whatsappTemplateName||'').trim().slice(0,100)||null,
    whatsappTemplateLanguage:String(req.body.whatsappTemplateLanguage||'en_US').trim().slice(0,30)||'en_US',
    whatsappBodyParams:Array.isArray(req.body.whatsappBodyParams)?req.body.whatsappBodyParams.map(x=>String(x).slice(0,200)).slice(0,20):[],
    stats:{total:campaignContacts({ownerId,filters,contactIds}).length,queued:0,sent:0,delivered:0,read:0,failed:0,replies:0,skipped:0}};
...
setInterval(async()=>{
  try{
    ...
    for(const c of db.data.campaigns.filter(x=>x.status==='draft'&&x.scheduledAt&&new Date(x.scheduledAt).getTime()<=now)){
      try{if(c.mode==='WHATSAPP')await dispatchWhatsAppCampaign(c);else dispatchInternalCampaign(c);}catch(e){c.status='failed';c.error=e.message;c.updatedAt=Date.now();db.save();}
    }
...
function classifyContact(c){
  ...
  // Expiry date is authoritative whenever present. This prevents stale manual
  // status values from overriding the calculated membership state.
  if(c.expiryDate){
  ...
  if(['exp','expire','expired','inactive','renewal_due'].includes(raw) || raw.includes('expired') || raw.includes('renew')) return 'expired';
  if(['active','valid','current'].includes(raw)) return 'valid';
  if(raw.includes('expiring') || raw.includes('soon')) return 'expiring_soon';
  return raw ? raw : 'unknown';
}
app.get('/api/integrations/status',authMiddleware,requireOwner,(req,res)=>{
  const missing=[];
  if(!db.isPostgres) missing.push('DATABASE_URL');
  if(!AI_MESSAGE_ENABLED) missing.push('AI_MESSAGE_API_KEY');
  if(!AI_CALLING_ENABLED){
    if(AI_CALL_PROVIDER==='vapi') missing.push('VAPI_PRIVATE_KEY/VAPI_API_KEY,VAPI_PHONE_NUMBER_ID,VAPI_ASSISTANT_ID');
    else if(AI_CALL_PROVIDER==='bolna') missing.push('BOLNA_API_KEY');
    else if(AI_CALL_PROVIDER==='bland') missing.push('BLAND_API_KEY');
    else missing.push('AI_CALL_PROVIDER + voice provider credentials');
  }
  if(!WHATSAPP_ENABLED) missing.push('WHATSAPP_API_VERSION,WHATSAPP_ACCESS_TOKEN,WHATSAPP_PHONE_NUMBER_ID');
  const turnConfigured=Boolean(TURN_SERVERS.length&&TURN_USERNAME&&TURN_CREDENTIAL);
  res.json({
    database:{connected:db.isPostgres,provider:db.isPostgres?'postgresql':'local-json'},
    aiMessaging:{connected:AI_MESSAGE_ENABLED,model:AI_MESSAGE_MODEL},
    aiCalling:{connected:AI_CALLING_ENABLED,provider:AI_CALL_PROVIDER||null},
    whatsapp:{connected:WHATSAPP_ENABLED,provider:'Meta WhatsApp Cloud API',templateRequired:true},
    webrtc:{turnConfigured,stunConfigured:STUN_SERVERS.length>0},
    missing
  });
});
app.get('/api/business/access',authMiddleware,(req,res)=>{
  const permissions=Object.fromEntries(Object.keys(DEFAULT_ROLE_PERMISSIONS.owner).map(k=>[k,hasBusinessPermission(req.user,k)]));
  res.json({role:req.user.role||'user',permissions,isOwner:isConfiguredOwner(req.user)});
});
...
app.patch('/api/owner/contacts/:id',authMiddleware,requireBusinessPermission('members'),(req,res)=>{
  ...
  db.saveNow().then(()=>{ audit(req.user.id,'contact.updated',{contactId:c.id}); res.json({contact:{...c,computedStatus:classifyContact(c)}}); }).catch(e=>res.status(500).json({error:'Member was updated but could not be saved permanently. Check DATABASE_URL.'}));
});
app.post('/api/owner/import-xlsx',authMiddleware,requireBusinessPermission('members'),(req,res,next)=>{
  ...
  uploadExcel.single('file')(req,res,err=>{
    ...
    try{
      ...
      db.saveNow().then(()=>{ audit(req.user.id,'contacts.xlsx_imported',{added,updated,skipped,rows:rows.length,sheetName,columns:COLUMNS.length});
      res.json({added,updated,skipped,totalRows:rows.length,sheetName,columns:COLUMNS});
      }).catch(e=>{ console.error('Excel persistence failed:', e); res.status(500).json({error:'Excel data was imported but could not be saved permanently. Please check DATABASE_URL.'}); });
...
app.post('/api/owner/message-templates',authMiddleware,requireBusinessPermission('campaigns'),(req,res)=>{
  ...
  const t={id:uuidv4(),ownerId:businessOwnerId(req.user),name:name.slice(0,100),body:body.slice(0,10000),mediaUrl:String(req.body.mediaUrl||'').trim().slice(0,2048)||null,mediaType:String(req.body.mediaType||'').slice(0,50)||null,
    whatsappTemplateName:String(req.body.whatsappTemplateName||'').trim().slice(0,100)||null,
    whatsappTemplateLanguage:String(req.body.whatsappTemplateLanguage||'en_US').trim().slice(0,30)||'en_US',
    whatsappBodyParams:Array.isArray(req.body.whatsappBodyParams)?req.body.whatsappBodyParams.map(x=>String(x).slice(0,200)).slice(0,20):[],
    enabled:req.body.enabled!==false,createdAt:Date.now(),updatedAt:Date.now()};
...
app.patch('/api/owner/message-templates/:id',authMiddleware,requireBusinessPermission('campaigns'),(req,res)=>{
  ...
  if(req.body.whatsappTemplateName!==undefined)t.whatsappTemplateName=String(req.body.whatsappTemplateName||'').trim().slice(0,100)||null;
  if(req.body.whatsappTemplateLanguage!==undefined)t.whatsappTemplateLanguage=String(req.body.whatsappTemplateLanguage||'en_US').trim().slice(0,30)||'en_US';
  if(req.body.whatsappBodyParams!==undefined)t.whatsappBodyParams=Array.isArray(req.body.whatsappBodyParams)?req.body.whatsappBodyParams.map(x=>String(x).slice(0,200)).slice(0,20):[];
...
function renderAiCallScript(script, contact){
  const vars={name:contact.name||'Member',status:classifyContact(contact),expiry_date:contact.expiryDate||'not available',renewal_time:contact.renewalTime||'',phone:contact.phone||'',member_id:contact.memberId||'',salon_name:contact.salonName||'',category:contact.category||'',city:contact.city||''};
  return String(script||'').replace(/\{([a-z0-9_]+)\}/gi,(m,k)=>Object.prototype.hasOwnProperty.call(vars,k.toLowerCase())?vars[k.toLowerCase()]:m);
}
...
function campaignEligibility(campaign){
  const all=db.data.marketingContacts.filter(c=>c.ownerId===campaign.ownerId);
  const targets=campaign.targetStatuses||[];
  const selected=all.filter(c=>targets.length===0||targets.includes(classifyContact(c)));
  const eligible=selected.filter(c=>c.optIn&&c.callingConsent===true&&c.phone);
  return {selected:selected.length,eligible:eligible.length,skippedOptOut:selected.filter(c=>!c.optIn).length,skippedNoConsent:selected.filter(c=>c.optIn&&c.callingConsent!==true).length,skippedNoPhone:selected.filter(c=>c.optIn&&c.callingConsent===true&&!c.phone).length};
}
function createCampaignJobs(campaign){
  ...
  const fixedScript=campaign.scriptId==='__AUTO__'?null:db.data.aiCallScripts.find(x=>x.id===campaign.scriptId && x.enabled);
  if(!fixedScript && campaign.scriptId!=='__AUTO__') throw new Error('Selected AI script is missing or disabled');
  let queued=0, skipped=0, skippedNoScript=0;
  for(const c of campaignCandidates(campaign)){
    if(existing.has(c.id)){skipped++;continue;}
    const script=fixedScript||db.data.aiCallScripts.find(x=>x.enabled&&x.matchStatus===classifyContact(c));
    if(!script){skippedNoScript++;continue;}
    db.data.aiCallJobs.push({id:uuidv4(), ownerId:campaign.ownerId, campaignId:campaign.id, contactId:c.id,phone:c.phone,name:c.name,status:'queued',provider:'not_configured',createdAt:Date.now(),attempts:0,scriptId:script.id,script:renderAiCallScript(script.script,c),memberStatus:classifyContact(c),expiryDate:c.expiryDate||null});
    queued++;
  }
  const eligibility=campaignEligibility(campaign);
  campaign.stats={...(campaign.stats||{}),...eligibility,queued,skipped,skippedNoScript,total:eligibility.selected};
  return {queued,skipped,skippedNoScript,...eligibility};
}
app.get('/api/owner/ai-calls/preview',authMiddleware,requireBusinessPermission('aiCalling'),(req,res)=>{
  ensureAutomationData();const statuses=normalizeStatuses(String(req.query.statuses||'').split(',').filter(Boolean));const ownerId=businessOwnerId(req.user);
  const all=db.data.marketingContacts.filter(c=>c.ownerId===ownerId);const selected=all.filter(c=>!statuses.length||statuses.includes(classifyContact(c)));
  res.json({selected:selected.length,eligible:selected.filter(c=>c.optIn&&c.callingConsent===true&&c.phone).length,skippedOptOut:selected.filter(c=>!c.optIn).length,skippedNoConsent:selected.filter(c=>c.optIn&&c.callingConsent!==true).length,skippedNoPhone:selected.filter(c=>c.optIn&&c.callingConsent===true&&!c.phone).length,byStatus:Object.fromEntries(['valid','expired','expiring_soon','new','unknown'].map(st=>[st,selected.filter(c=>classifyContact(c)===st).length]))});
});
...
app.post('/api/owner/ai-calls/campaigns',authMiddleware,requireBusinessPermission('aiCalling'),(req,res)=>{
  ...
  const autoScript=scriptId==='__AUTO__';
  const script=autoScript?null:db.data.aiCallScripts.find(x=>x.id===scriptId&&x.enabled);
  if(!name||(!script&&!autoScript))return res.status(400).json({error:'Campaign name and an enabled AI script are required, or choose Auto-match by status'});
...
app.post('/api/owner/ai-calls/campaigns/:id/start',authMiddleware,requireBusinessPermission('aiCalling'),(req,res)=>{ensureAutomationData();const c=db.data.aiCallCampaigns.find(x=>x.id===req.params.id&&x.ownerId===businessOwnerId(req.user));if(!c)return res.status(404).json({error:'AI calling campaign not found'});if(['completed','cancelled'].includes(c.status))return res.status(409).json({error:'Campaign cannot be started in its current state'});c.startAt=Date.now();if(c.endAt<=c.startAt)c.endAt=c.startAt+60*60*1000;c.status='scheduled';c.error=null;db.save();res.json({campaign:c});});
app.post('/api/owner/ai-calls/campaigns/:id/pause',authMiddleware,requireBusinessPermission('aiCalling'),(req,res)=>{ensureAutomationData();const c=db.data.aiCallCampaigns.find(x=>x.id===req.params.id&&x.ownerId===businessOwnerId(req.user));if(!c)return res.status(404).json({error:'AI calling campaign not found'});if(!['scheduled','running'].includes(c.status))return res.status(409).json({error:'Only scheduled/running campaigns can be paused'});c.status='paused';c.pausedAt=Date.now();db.data.aiCallJobs.filter(j=>j.campaignId===c.id&&j.status==='queued').forEach(j=>{j.status='paused';j.updatedAt=Date.now();});db.save();res.json({campaign:c});});
app.post('/api/owner/ai-calls/campaigns/:id/resume',authMiddleware,requireBusinessPermission('aiCalling'),(req,res)=>{ensureAutomationData();const c=db.data.aiCallCampaigns.find(x=>x.id===req.params.id&&x.ownerId===businessOwnerId(req.user));if(!c)return res.status(404).json({error:'AI calling campaign not found'});if(c.status!=='paused')return res.status(409).json({error:'Campaign is not paused'});c.status='scheduled';c.startAt=Date.now();db.data.aiCallJobs.filter(j=>j.campaignId===c.id&&j.status==='paused').forEach(j=>{j.status='queued';j.updatedAt=Date.now();});db.save();res.json({campaign:c});});
...
async function processAiCallScheduler(){
  ...
  for(const c of db.data.aiCallCampaigns){
    if(['cancelled','completed','failed','paused'].includes(c.status))continue;
...
app.post('/api/webhooks/ai-calls/bland/:jobId', express.json({limit:'1mb'}), (req,res)=>{
  ...
  j.outcome = body.answered_by==='voicemail' ? 'no_answer' : (body.completed===false ? 'failed' : (body.outcome || 'answered'));
  j.interest = body.interest || body.interested || null;
...
app.post('/api/webhooks/ai-calls/bolna', express.json({limit:'1mb'}), (req,res)=>{
  ...
  j.outcome = body.answered_by_voice_mail ? 'no_answer' : (body.status==='error' ? 'failed' : (body.outcome || 'answered'));
  j.interest = body.interested ?? body.interest ?? null;
...
app.post('/api/webhooks/ai-calls/vapi',express.json({limit:'2mb'}),(req,res)=>{
  ensureAutomationData();const body=req.body||{};const callId=String(body.call?.id||body.id||'');const j=db.data.aiCallJobs.find(x=>String(x.providerCallId)===callId);if(!j)return res.status(404).json({error:'Unknown call'});
  const status=String(body.status||body.call?.status||'').toLowerCase();if(['ended','completed','hangup'].includes(status))j.status='completed';else if(['failed','error'].includes(status))j.status='failed';
  j.transcript=body.transcript||body.call?.transcript||j.transcript||null;j.summary=body.summary||body.call?.summary||j.summary||null;j.outcome=body.outcome||j.outcome||(j.status==='failed'?'failed':'answered');j.completedAt=Date.now();j.updatedAt=Date.now();db.save();audit(j.ownerId,'ai_call.vapi_webhook',{jobId:j.id,status:j.status});res.json({ok:true});
});
...
app.get('/api/business/users',authMiddleware,requireOwner,(req,res)=>{
  res.json({users:db.data.users.map(u=>({id:u.id,name:u.name,username:u.username,phone:u.phone,email:u.email||null,avatar:u.avatar||null,createdAt:u.createdAt||null,lastSeen:u.lastSeen||null,online:typeof onlineUsers!=='undefined' ? onlineUsers.has(u.id) : false,role:u.role||'user',permissions:u.permissions||{}}))});
});
...
function resolveCampaignVariable(value, contact, event){
  const vars={name:contact.name||'Customer',status:classifyContact(contact),expiry_date:contact.expiryDate||'',renewal_time:contact.renewalTime||'',
    member_id:contact.memberId||'',phone:contact.phone||'',salon_name:contact.salonName||'',category:contact.category||'',
    artist:contact.artist||event?.artist||'',topic:contact.topic||event?.topic||'',event:event?.name||'',class_name:event?.name||'',
    date:event?.date||'',time:event?.time||'',location:event?.location||'',zoom_link:event?.zoomLink||''};
  return String(value??'').replace(/\{([a-z0-9_]+)\}/gi,(m,k)=>Object.prototype.hasOwnProperty.call(vars,k.toLowerCase())?vars[k.toLowerCase()]:m);
}
function renderWhatsAppBodyParams(campaign, contact, event, template){
  const source=Array.isArray(campaign.whatsappBodyParams)&&campaign.whatsappBodyParams.length?campaign.whatsappBodyParams:(Array.isArray(template?.whatsappBodyParams)?template.whatsappBodyParams:[]);
  return source.map(x=>resolveCampaignVariable(x,contact,event));
}
async function sendWhatsAppTemplateMessage(campaign, contact, event, template){
  if(!WHATSAPP_ENABLED) throw new Error('WhatsApp Business API is not connected. Configure WHATSAPP_API_VERSION, WHATSAPP_ACCESS_TOKEN and WHATSAPP_PHONE_NUMBER_ID.');
  const templateName=campaign.whatsappTemplateName||template?.whatsappTemplateName;
  const language=campaign.whatsappTemplateLanguage||template?.whatsappTemplateLanguage||'en_US';
  if(!templateName) throw new Error('An approved WhatsApp template name is required for business-initiated campaigns.');
  const bodyParams=renderWhatsAppBodyParams(campaign,contact,event,template);
  const components=[];const mediaUrlRaw=campaign.mediaUrl||template?.mediaUrl||null;const mediaUrl=mediaUrlRaw?.startsWith('/')?`${APP_BASE_URL}${mediaUrlRaw}`:mediaUrlRaw;const mediaType=campaign.mediaType||template?.mediaType||null;
  if(mediaUrl&&['image','video','document'].includes(mediaType))components.push({type:'header',parameters:[{type:mediaType,[mediaType]:{link:mediaUrl}}]});
  if(bodyParams.length)components.push({type:'body',parameters:bodyParams.map(text=>({type:'text',text}))});
  const payload={messaging_product:'whatsapp',to:normalizePhone(contact.phone).replace(/^\+/,''),type:'template',template:{name:templateName,language:{code:language},components}};
  const url=`https://graph.facebook.com/${WHATSAPP_API_VERSION}/${WHATSAPP_PHONE_NUMBER_ID}/messages`;
  const resp=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${WHATSAPP_ACCESS_TOKEN}`},body:JSON.stringify(payload)});
  const data=await resp.json().catch(()=>({}));
  if(!resp.ok) throw new Error(data.error?.message||data.message||`WhatsApp API returned ${resp.status}`);
  return data.messages?.[0]?.id||null;
}
async function dispatchWhatsAppCampaign(campaign){
  if(!WHATSAPP_ENABLED) throw new Error('WhatsApp Business API is not connected. Configure WHATSAPP_API_VERSION, WHATSAPP_ACCESS_TOKEN and WHATSAPP_PHONE_NUMBER_ID.');
  const contacts=campaignContacts(campaign);
  const template=campaign.templateId?db.data.messageTemplates.find(t=>t.id===campaign.templateId&&t.ownerId===campaign.ownerId):null;
  const event=campaign.eventId?db.data.events.find(e=>e.id===campaign.eventId&&e.ownerId===campaign.ownerId):null;
  if(campaign.eventId && (!event || ['completed','cancelled','archived'].includes(event.status))) throw new Error('This event is no longer active.');
  let accepted=0,failed=0,skipped=0; campaign.stats={total:contacts.length,queued:contacts.length,sent:0,delivered:0,read:0,failed:0,replies:0,skipped:0}; campaign.whatsappMessageIds=[];
  for(const c of contacts){
    if(!c.phone){skipped++;continue;}
    try{const id=await sendWhatsAppTemplateMessage(campaign,c,event,template);if(id)campaign.whatsappMessageIds.push(id);c.lastWhatsAppSentAt=Date.now();c.lastWhatsAppMessageId=id||null;accepted++;}catch(e){failed++;}
  }
  campaign.stats.queued=0;campaign.stats.sent=accepted;campaign.stats.failed=failed;campaign.stats.skipped=skipped;campaign.status=failed&&accepted===0?'failed':'completed';campaign.mode='WHATSAPP';campaign.completedAt=Date.now();campaign.updatedAt=Date.now();db.save();audit(campaign.ownerId,'campaign.whatsapp_completed',{campaignId:campaign.id,accepted,failed,skipped,total:contacts.length});
  return {accepted,failed,skipped,total:contacts.length,provider:'meta_whatsapp'};
}
app.get('/api/marketing/whatsapp/status',authMiddleware,requireBusinessPermission('campaigns'),(req,res)=>res.json({connected:WHATSAPP_ENABLED,provider:'Meta WhatsApp Cloud API',templateRequired:true}));
app.post('/api/marketing/campaigns/:id/send-whatsapp',authMiddleware,requireBusinessPermission('campaigns'),async(req,res)=>{
  marketingData();const c=db.data.campaigns.find(x=>x.id===req.params.id&&x.ownerId===businessOwnerId(req.user));if(!c)return res.status(404).json({error:'Campaign not found'});if(c.status==='completed')return res.status(409).json({error:'Campaign already completed'});
  try{const result=await dispatchWhatsAppCampaign(c);res.json({campaign:c,result});}catch(e){c.status='blocked';c.error=e.message;c.updatedAt=Date.now();db.save();res.status(409).json({error:e.message,campaign:c});}
});
app.get('/api/webhooks/whatsapp',(req,res)=>{const mode=req.query['hub.mode'],token=req.query['hub.verify_token'],challenge=req.query['hub.challenge'];if(mode==='subscribe'&&WHATSAPP_VERIFY_TOKEN&&token===WHATSAPP_VERIFY_TOKEN)return res.status(200).send(String(challenge||''));return res.sendStatus(403);});
app.post('/api/webhooks/whatsapp',express.json({limit:'2mb'}),(req,res)=>{for(const entry of (req.body?.entry||[]))for(const change of (entry.changes||[])){for(const status of (change.value?.statuses||[])){const id=String(status.id||'');for(const campaign of (db.data.campaigns||[])){if(!campaign.whatsappMessageIds?.includes(id))continue;campaign.stats=campaign.stats||{};if(status.status==='delivered')campaign.stats.delivered=(campaign.stats.delivered||0)+1;if(status.status==='read')campaign.stats.read=(campaign.stats.read||0)+1;if(status.status==='failed')campaign.stats.failed=(campaign.stats.failed||0)+1;}}}db.save();res.json({ok:true});});
...
// Public WebRTC configuration contains only non-secret ICE server information.
app.get('/api/calls/config',authMiddleware,(req,res)=>{
  const iceServers=[...STUN_SERVERS.map(urls=>({urls}))];
  if(TURN_SERVERS.length && TURN_USERNAME && TURN_CREDENTIAL) iceServers.push({urls:TURN_SERVERS,username:TURN_USERNAME,credential:TURN_CREDENTIAL});
  res.json({iceServers,turnConfigured:Boolean(TURN_SERVERS.length&&TURN_USERNAME&&TURN_CREDENTIAL)});
});
...
if(!onlineUsers.has(userId))onlineUsers.set(userId,new Set());onlineUsers.get(userId).add(socket.id);socket.join(`user:${userId}`);db.data.chats.filter(c=>c.memberIds.includes(userId)).forEach(c=>socket.join(c.id));io.emit('presence:online',{userId});
  socket.on('message:send',({chatId,text,mediaUrl,mediaType,replyTo,attachmentType,attachmentData}={})=>{
    const chat=db.data.chats.find(c=>c.id===chatId);if(!chat||!chat.memberIds.includes(userId))return;
    if(chat.type==='group'&&chat.permissions?.onlyAdminsCanMessage&&!chat.adminIds.includes(userId))return socket.emit('error:message',{error:'Only admins can send messages in this group'});
    const blocked=chat.memberIds.some(id=>id!==userId&&db.data.users.find(u=>u.id===id)?.blockedUserIds?.includes(userId));if(blocked)return socket.emit('error:message',{error:'Messaging is blocked'});
    const safeText=String(text||'').slice(0,5000);const allowedAttachments=['file','image','video','audio','document','contact','poll','event','sticker'];
    const type=allowedAttachments.includes(String(attachmentType||''))?String(attachmentType):null;
    if(!safeText&&!mediaUrl&&!type)return;
    const safeData=attachmentData&&typeof attachmentData==='object'?JSON.parse(JSON.stringify(attachmentData)):null;
    const m={id:uuidv4(),chatId,senderId:userId,text:safeText,mediaUrl:mediaUrl||null,mediaType:mediaType||null,attachmentType:type,attachmentData:safeData,replyTo:replyTo||null,reactions:{},edited:false,createdAt:Date.now(),deliveredTo:[userId],readBy:[userId],deletedFor:[]};
    db.data.messages.push(m);db.save();io.to(chatId).emit('message:new',m);
  });
  socket.on('message:poll-vote',({messageId,optionIndex}={})=>{const m=db.data.messages.find(x=>x.id===messageId),chat=m&&db.data.chats.find(c=>c.id===m.chatId);if(!m||!chat?.memberIds.includes(userId)||m.attachmentType!=='poll')return;const opts=m.attachmentData?.options;if(!Array.isArray(opts)||!Number.isInteger(optionIndex)||!opts[optionIndex])return;const votes=m.attachmentData.votes||{};votes[userId]=optionIndex;m.attachmentData.votes=votes;db.save();io.to(chat.id).emit('message:poll-updated',{messageId,attachmentData:m.attachmentData});});
  socket.on('message:event-rsvp',({messageId,response}={})=>{const m=db.data.messages.find(x=>x.id===messageId),chat=m&&db.data.chats.find(c=>c.id===m.chatId);if(!m||!chat?.memberIds.includes(userId)||m.attachmentType!=='event')return;const allowed=['going','maybe','not_going'];if(!allowed.includes(response))return;const rsvps=m.attachmentData.rsvps||{};rsvps[userId]=response;m.attachmentData.rsvps=rsvps;db.save();io.to(chat.id).emit('message:event-updated',{messageId,attachmentData:m.attachmentData});});
